...
const generateROCCurve = (modelName: string, baseAUC: number) => {
  ...
  for (let i = 0; i <= numPoints; i++) {
    ...
    let tpr;
    if (baseAUC > 0.95) {
      // Curva muy buena - sube rápido al inicio
...
const modelResults = {
  ...
  gradientBoosting: {
    ...
    accuracy: 0.88,
    metrics: {
      noConfirmado: { precision: 0.94, recall: 0.89, f1: 0.92, support: 1292 },
      confirmado: { precision: 0.78, recall: 0.86, f1: 0.82, support: 549 }
    }
  },
  xgboost: {
    auc: 0.9570,
...
// ============================================
// DATA GENERATION WITH REAL METRICS
// ============================================

const generateSimulatedData = (numSamples = 150) => {
  const data = [];
  
  // Usar proporciones reales: 1292 No Confirmado, 549 Confirmado
  const totalReal = 1841;
  const confirmedRatio = 549 / totalReal; // ~30%
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    if (isConfirmed) {
      // Confirmados: mayor SNR, parámetros más definidos
      snr = 15 + Math.random() * 35; // SNR alto
      period = Math.exp(Math.random() * 4 + 1);
      depth = Math.exp(Math.random() * 2 - 4);
      prad = 1 + Math.random() * 10; // Radio entre 1-11 R⊕
      // Basado en recall=0.85 del mejor modelo (XGBoost)
      confidence = Math.random() < 0.87 ? 0.75 + Math.random() * 0.25 : 0.3 + Math.random() * 0.45;
    } else {
      // No Confirmados: SNR variable, más dispersión
      snr = 3 + Math.random() * 25;
      period = Math.exp(Math.random() * 6);
      depth = Math.exp(Math.random() * 4 - 6);
      prad = 0.5 + Math.random() * 15;
      // Basado en recall=0.91 del mejor modelo
...
// ============================================
// ROC CURVE INTERACTIVE COMPONENT
// ============================================
      ...
      <ResponsiveContainer width="100%" height={400}>
        <LineChart margin={{ top: 20, right: 30, left: 60, bottom: 40 }}>
          ...
          <Tooltip
            ...
            labelFormatter={(value) => `Umbral: ${(value as number).toFixed(3)}`}
          />
          <Line
            type="monotone"
...
const ModelComparison = () => {
  const comparisonData = [
    {
      model: 'Random\nForest',
      auc: modelResults.randomForest.auc,
      accuracy: modelResults.randomForest.accuracy,
      f1_confirmado: modelResults.randomForest.metrics.confirmado.f1,
      f1_no_confirmado: modelResults.randomForest.metrics.noConfirmado.f1
    },
    {
      model: 'Gradient\nBoosting',
      auc: modelResults.gradientBoosting.auc,
      accuracy: modelResults.gradientBoosting.accuracy,
      f1_confirmado: modelResults.gradientBoosting.metrics.confirmado.f1,
      f1_no_confirmado: modelResults.gradientBoosting.metrics.noConfirmado.f1
    },
    {
      model: 'XGBoost\n(Mejor)',
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <div style={{
        ...
        background: '#1a1a2e',
        borderRadius: '8px',
        border: '2px solid #7b2ff740',
        fontSize: '13px',
        color: '#8b92a8'
      }}>
        <div style={{ color: '#7b2ff7', fontWeight: 'bold', marginBottom: '8px' }}>
          🥇 Mejor Modelo: XGBoost
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          <div>• <strong style={{ color: '#00d4ff' }}>AUC:</strong> 0.9570 (95.7%)</div>
          <div>• <strong style={{ color: '#00d4ff' }}>Accuracy:</strong> 90%</div>
          <div>• <strong style={{ color: '#00ff88' }}>Recall Confirmado:</strong> 87%</div>
          <div>• <strong style={{ color: '#ff4444' }}>Precision No Conf.:</strong> 94%</div>
        </div>
      </div>
    </div>
  );
...
// ============================================
// CLASSIFICATION SCATTER WITH REAL DATA
// ============================================
...
export function Status({ onBackToHome }: StatusProps) {
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '30px' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          <h1 style={{
            ...
            🚀 NASA Exoplanet AI Challenge
          </h1>
          <p style={{ color: '#8b92a8', fontSize: '16px' }}>
            Resultados del Sistema de Clasificación con Machine Learning
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <MetricsCard 
          icon="🎯"
          title="Accuracy"
          value={`${(modelResults.xgboost.accuracy * 100).toFixed(0)}%`}
          subtitle="1654 de 1841 correctos"
          color="#00d4ff"
          delay={1.5}
        />
        <MetricsCard 
          icon="✅"
          title="Recall Confirmados"
          value="87%"
          subtitle="478 de 549 detectados"
          color="#00ff88"
          delay={3}
        />
        <MetricsCard 
          icon="🛡️"
          title="Precision No Conf."
          value="94%"
          subtitle="Baja tasa de falsos positivos"
          color="#ff4444"
          delay={4.5}
        />
      </div>
      ...
      <div style={{
        ...
        <p style={{ color: '#8b92a8', fontSize: '14px', margin: 0 }}>
          💡 <strong>Sobre el Modelo:</strong> Entrenado con 1841 exoplanetas del catálogo Kepler • 
          3 modelos comparados (RF, GB, XGBoost) • 10 características principales • 
